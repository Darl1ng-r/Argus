...
-- Covering index for atomic vote calculations (Index-Only Scan)
CREATE INDEX IF NOT EXISTS idx_votes_covering
    ON votes(node_id, vote_type, user_id);

-- Partial index for fast Steelman dialectic path extraction
CREATE INDEX IF NOT EXISTS idx_nodes_topic_steel
    ON nodes(topic_id, is_steel, support_score DESC)
    WHERE status = 'ACTIVE';

-- Composite index for explore feed pagination with privacy filter
