//#region node_modules/cytoscape-dagre/dist/cytoscape-dagre.mjs
/*!
...
var assign = Object.assign != null ? Object.assign.bind(Object) : function(tgt) {
	for (var _len = arguments.length, srcs = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) srcs[_key - 1] = arguments[_key];
...
function buildEdgeFrame(src, tgt) {
	var _norm = norm(subtract(tgt, src)), x = _norm.x, y = _norm.y, len = _norm.len;
	var dir = {
		x,
		y
	};
...
function noZero(x) {
	if (Math.abs(x) < EPSILON) return x < 0 ? -EPSILON : EPSILON;
	return x;
}
...
function dagreEdgeToCytoscapeEdge(dEdge, cEdge) {
	var frame = buildEdgeFrame(cEdge.source().position(), cEdge.target().position());
	var coords = normalizeWeight(dEdge.points.map(function(p) {
		return toEdgeCoordinates(p, frame);
	}));
	return {
		controlPointWeights: coords.slice(1, -1).map(function(c) {
			return c.weight;
		}),
		controlPointDistances: coords.slice(1, -1).map(function(c) {
			return c.distance;
		})
	};
}
DagreLayout.prototype.run = function() {
	...
	for (var _i = 0; _i < nodes.length; _i++) {
		var _node = nodes[_i];
		if (_node.isChild()) g.setParent(_node.id(), _node.parent().id());
	}
